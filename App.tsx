      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            // FIX: Smoother transition for mobile sidebar (removed spring)
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark }: any) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.visualViewport) {
        ...
        setViewportStyle({ height: `${window.innerHeight}px`, top: '0px' });
      }
    };
  ...
  // FIX: Force focus back to editor before executing commands to solve button click issues
  ...
  return (
    // FIX: Theme class applied to ROOT container for seamless look
    <div 
        style={viewportStyle}
        className={`fixed left-0 w-full z-[100] flex flex-col ${themeClass}`}
    >
       {/* FIX: Transparent Header to blend with body */}
       <div className={`flex-none flex justify-between items-center p-4 border-b border-black/5`}>
          <button onClick={onClose} className={`p-3 rounded-full hover:bg-black/5`}><ArrowLeft/></button>
          <div className="flex gap-2 relative">
             <IconButton icon={Palette} onClick={() => setShowSettings(!showSettings)} />
             <IconButton icon={Trash2} onClick={onDelete} className="text-red-600 hover:bg-red-500/20" />
             {showSettings && (
       ...
       {/* Content Container - Transparent to show Root Theme */}
       ...
       <div className={`flex-none w-full p-2 border-t border-black/5 ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`}>
