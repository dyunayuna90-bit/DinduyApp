...
import { 
  ...
  CheckCircle2, Check, Minimize2, Maximize2, Search, Heart, Menu, Download, Upload, CheckSquare, AlertCircle
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      >
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="opacity-60 text-sm">Item ini akan dihapus permanen dan tidak dapat dikembalikan.</p>
        </div>

        <div className="flex items-center gap-2 mb-6 justify-center opacity-70 cursor-pointer" onClick={() => setSuppress(!suppress)}>
           <div className={`w-5 h-5 rounded border flex items-center justify-center ${suppress ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-current'}`}>
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div 
               ...
               className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[80vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}
            >
                        ...
                        {Object.keys(colors).map(c => {
                             ...
                             return (
                                <button 
                                  ...
                                  className={`w-10 h-10 rounded-full border-2 ${bgClass} ${folder.color === c ? 'border-indigo-500 scale-110' : 'border-transparent'}`}
                                />
                             )
                        })}
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {SHAPES.map((s,i) => (
                            <button 
                               ...
                               className={`h-12 bg-current opacity-10 hover:opacity-20 transition-opacity ${s} ${folder.shape === s ? '!opacity-40 ring-2 ring-indigo-500' : ''}`}
                            />
                        ))}
                    </div>
                </div>
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            deleteSelected={() => {
                // Bulk delete doesn't need swipe confirmation, but could leverage same modal if desired. 
                // For simplicity, execute immediately or add bulk confirmation here.
  ...
  return (
    ...
    >
        <div className="relative w-full h-full rounded-3xl">
            {/* NO RED BACKGROUND LAYER - CLEAN SWIPE */}
            <motion.div
            ...
            >
                ...
                <div className="p-6">
                    <div className="flex justify-between items-start">
                        ...
                        {/* Settings Button - Triggers Global Modal */}
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark }: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col ${isDark ? 'bg-[#121212]' : 'bg-[#fdfcf4]'}`}>
       {/* FIXED HEADER */}
       <div className={`flex-none flex justify-between items-center p-4 border-b border-black/5 ${isDark ? 'bg-[#121212]' : 'bg-white'}`}>
       ...
       {/* SCROLLABLE CONTENT */}
       <div className={`flex-1 overflow-y-auto px-6 py-6 pb-32 ${themeClass}`}>
       ...
       <div className={`flex-none w-full p-2 border-t border-black/5 pb-[env(safe-area-inset-bottom)] ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`}>
