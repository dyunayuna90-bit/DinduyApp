...
import { 
  ...
  CheckSquare, AlertCircle, Plus, CornerDownLeft, Undo // <-- FIX: Import Undo Icon
} from 'lucide-react';
...
export default function App() {
  ...
  // --- FIX: SATPAM HISTORY (Mencegah loop history saat back ditekan) ---
  const historyPopped = useRef(false);

  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      // TANDAI BAHWA KITA SEDANG BACK (JANGAN PUSH LAGI)
      ...
      // 3. Tutup Note Mengembang
      else if (peekingNoteId) {
        setPeekingNoteId(null);
        event.preventDefault();
      }
      // 4. Tutup Folder Mengembang
      else if (expandedFolderId) {
        setExpandedFolderId(null);
        event.preventDefault();
      }
      // 5. Matikan Mode Select
  ...
  // --- LOGIKA PUSH STATE YANG LEBIH PINTAR ---
  useEffect(() => {
    // Kalau perubahan terjadi karena tombol Back, JANGAN nambah history
    if (historyPopped.current) {
      historyPopped.current = false; // Reset satpam
      return;
    }

    // Kalau user membuka sesuatu, baru kita tambah history
    if (activeNoteId || showSidebar || peekingNoteId || expandedFolderId || isSelectionMode) {
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            peekingNoteId={peekingNoteId} // PASS STATE
            setPeekingNoteId={setPeekingNoteId} // PASS SETTER
        ...
        <motion.div layout transition={springTransition} className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
           {searchQuery ? filteredNotes.map((note: Note) => (
               <NoteCard 
                  ...
                  // Kalau search, kita disable peeking dulu biar simpel, atau pake state lokal di card (search mode beda behaviour)
               />
           )) : (
             <>
