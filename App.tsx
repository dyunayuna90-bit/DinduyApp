                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {SHAPES.slice(0, 12).map((s,i) => (
                            <button 
                               ...
                               className={`h-12 bg-current opacity-10 hover:opacity-20 transition-opacity ${s} ${note.shape === s ? '!opacity-40 ring-2 ring-indigo-500' : ''}`}
                            />
                        ))}
                    </div>
                </div>
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark }: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col ${themeClass}`}>
       ...
       <div className={`flex-none flex justify-between items-center p-4 border-b border-black/5 bg-transparent`}>
          <button onClick={onClose} className={`p-3 rounded-full hover:bg-black/5`}><ArrowLeft/></button>
          <div className="flex gap-2 relative">
             <IconButton icon={Palette} onClick={() => setShowSettings(true)} />
       ...
       {/* FIX 2: Toolbar Pake justify-start biar gak kepotong (ditambah padding) */}
...
export default function App() {
  ...
  // --- LOGIKA TOMBOL BACK ANDROID (ANTI-CLOSE) ---
  useEffect(() => {
    // Fungsi ini jalan pas user tekan tombol Back HP
    const handlePopState = (event: PopStateEvent) => {
      // 1. Kalau Catatan lagi kebuka -> Tutup Catatan
      if (activeNoteId) {
        setActiveNoteId(null);
        event.preventDefault(); // Cegah aplikasi keluar
      } 
      // 2. Kalau Sidebar lagi kebuka -> Tutup Sidebar
      else if (showSidebar) {
        setShowSidebar(false);
        event.preventDefault();
      }
      // 3. Kalau Modal Select lagi aktif -> Matiin Select
      else if (isSelectionMode) {
        setIsSelectionMode(false);
        setSelectedIds(new Set());
        event.preventDefault();
      }
      // 4. Kalau Menu Tambah (+) lagi buka -> Tutup
      else if (showNewMenu) {
    ...
    // Pasang kuping buat dengerin tombol Back
    window.addEventListener('popstate', handlePopState);

    // Bersihin kalau komponen di-refresh
    return () => window.removeEventListener('popstate', handlePopState);
  }, [activeNoteId, showSidebar, isSelectionMode, showNewMenu]);

  // --- LOGIKA "PUSH STATE" (Biar HP tau kita pindah 'halaman') ---
  // Setiap kali kita buka Catatan atau Sidebar, kita "pura-pura" pindah halaman
  useEffect(() => {
    if (activeNoteId || showSidebar || isSelectionMode) {
      // Masukin "history palsu" ke browser
      window.history.pushState(null, '', window.location.href);
    }
  }, [activeNoteId, showSidebar, isSelectionMode]);
