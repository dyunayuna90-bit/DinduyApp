...
import { 
  Plus, ArrowLeft, Bold, Italic, Underline, List, AlignLeft, AlignCenter, AlignRight,
  FolderPlus, FilePlus, Trash2, X, Palette, Sun, Moon, Folder as FolderIcon,
  CheckCircle2, Check, Minimize2, Maximize2, Search, Heart, Menu, Download, Upload, CheckSquare
} from 'lucide-react';
...
const ToolbarButton = ({ onClick, icon: Icon, active = false, isDark }: any) => (
  <button 
    ...
    className={`p-3 rounded-xl transition-colors ${active ? 'bg-indigo-600 text-white' : isDark ? 'text-zinc-400 hover:text-white hover:bg-zinc-800' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-200'}`}
  >
    <Icon size={20} />
  </button>
);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className={`fixed top-0 left-0 bottom-0 w-80 z-[70] p-6 shadow-2xl overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
            ...
            <div className="space-y-4">
              <button onClick={toggleTheme} className={`w-full flex items-center gap-3 p-4 rounded-xl transition-colors border ${isDark ? 'border-zinc-700 hover:bg-zinc-800' : 'border-zinc-200 hover:bg-zinc-50'}`}>
                {isDark ? <Sun className="text-yellow-400" /> : <Moon className="text-indigo-600" />}
                <span className="font-medium">{isDark ? 'Light Mode' : 'Dark Mode'}</span>
              </button>
              <button onClick={onExport} className={`w-full flex items-center gap-3 p-4 rounded-xl transition-colors border ${isDark ? 'border-zinc-700 hover:bg-zinc-800' : 'border-zinc-200 hover:bg-zinc-50'}`}>
                <Download size={20} />
                <span className="font-medium">Backup Data</span>
              </button>
              <button onClick={() => fileInputRef.current?.click()} className={`w-full flex items-center gap-3 p-4 rounded-xl transition-colors border ${isDark ? 'border-zinc-700 hover:bg-zinc-800' : 'border-zinc-200 hover:bg-zinc-50'}`}>
                <Upload size={20} />
    ...
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          <NoteEditor 
            ...
            onDelete={() => { setNotes(prev => prev.filter(n => n.id !== activeNoteId)); setActiveNoteId(null); }}
      ...
      {!activeNoteId && !isSelectionMode && !searchQuery && (
        <div className="fixed bottom-6 right-6 z-40">
           <AnimatePresence>
            {showNewMenu && (
              <motion.div initial={{ opacity: 0, scale: 0.8, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 10 }} className="absolute bottom-20 right-0 flex flex-col gap-3 items-end mb-2">
           ...
           <motion.button whileTap={{ scale: 0.9 }} onClick={() => setShowNewMenu(!showNewMenu)} className={`w-16 h-16 rounded-[1.2rem] shadow-xl flex items-center justify-center border-2 ${isDark ? 'bg-emerald-800 border-emerald-700 text-white' : 'bg-zinc-900 border-zinc-900 text-white'}`}>
    ...
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) setIsSelectionMode(false); }}>
      <header className="mb-8 flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
         ...
         <div className={`flex-1 relative rounded-2xl flex items-center px-4 py-3 border-2 transition-colors ${isDark ? 'bg-zinc-900 border-zinc-700 focus-within:border-zinc-500' : 'bg-white border-zinc-200 focus-within:border-zinc-400'}`}>
            ...
            <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search notes..." className="bg-transparent outline-none w-full placeholder-current/50 font-medium" />
            {searchQuery && <button onClick={() => setSearchQuery('')}><X size={16} /></button>}
         </div>
         <button onClick={() => setIsSelectionMode(!isSelectionMode)} className={`p-3 rounded-xl transition-colors ${isSelectionMode ? 'bg-indigo-600 text-white' : isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-white hover:bg-zinc-100 border border-zinc-200'}`}>
            <CheckSquare size={24} />
         </button>
      </header>
...
const FolderItem = ({ 
  ...
  useEffect(() => {
    if (!isExpanded) { setShowSettings(false); setEditing(false); }
  }, [isExpanded]);
  ...
  return (
    ...
    >
        <div className="relative w-full h-full rounded-3xl">
            {/* Red Background (Trash) - Bidirectional */}
            {!isExpanded && !isSelectionMode && (
                <div className="absolute inset-0 bg-red-500 rounded-[2rem] z-0 flex items-center justify-between px-6">
            ...
            <motion.div
                ...
                dragElastic={{ left: 0.5, right: 0.5 }} // Bidirectional
            ...
            >
                ...
                <div className="p-6">
                    <div className="flex justify-between items-start">
                        {editing ? (
                            <input 
                                ref={inputRef} defaultValue={folder.name} 
                        ...
                        {isExpanded && !isSelectionMode && (
                            <div className="relative">
                                ...
                                {showSettings && (
                                    <div className={`absolute top-12 right-0 w-72 max-h-[300px] overflow-y-auto p-4 rounded-2xl shadow-xl z-50 border-2 cursor-auto ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`} onClick={e => e.stopPropagation()}>
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {Object.keys(colors).map(c => {
                                                const bgClass = LIGHT_COLORS[c as ColorTheme].split(' ')[0]; // Extract bg class
                                                return (
                                                  <button 
                                                    key={c} onClick={() => onUpdate(folder.id, {color: c})} 
                                                    className={`w-8 h-8 rounded-full border border-black/10 ${bgClass}`} 
                                                  />
                                                );
                                            })}
                                        </div>
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode }: any) => {
  const Icon = ICONS[note.icon] || ICONS['file-text'];
  const themeClass = colors[note.color] || colors.slate;
  const [isPeeking, setIsPeeking] = useState(false);

  useEffect(() => { if (!isPeeking) { /* reset interactions if needed */ } }, [isPeeking]);
  ...
  return (
    ...
    >
        <div className="relative w-full h-full">
            {/* Trash Background - Bidirectional */}
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark }: any) => {
  ...
  return (
    <motion.div layoutId={`note-${note.id}`} className={`fixed inset-0 z-[100] flex flex-col ${isDark ? 'bg-[#121212]' : 'bg-[#fdfcf4]'} md:p-6`}>
       <motion.div className={`flex-1 flex flex-col w-full max-w-4xl mx-auto md:rounded-[2.5rem] overflow-hidden shadow-2xl border-2 ${themeClass} ${isDark ? 'md:border-zinc-700' : 'md:border-zinc-200'} pb-20`}>
          <div className={`flex justify-between items-center p-4 border-b border-black/5 ${isDark ? 'bg-[#121212]' : 'bg-white'}`}>
             ...
             <div className="flex gap-2 relative">
                ...
                {showSettings && (
                   <div className={`absolute top-14 right-0 w-80 max-h-[400px] overflow-y-auto p-4 rounded-3xl shadow-xl z-50 border-2 ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`}>
                      <div className="mb-4">
                         <h4 className="text-xs font-bold opacity-50 mb-2 uppercase">Color</h4>
                         <div className="flex flex-wrap gap-2">{Object.keys(colors).map(c => {
                             const bgClass = LIGHT_COLORS[c as ColorTheme].split(' ')[0];
                             return <button key={c} onClick={() => onUpdate(note.id, {color: c})} className={`w-8 h-8 rounded-full border border-black/10 ${bgClass}`} />
                         })}</div>
                      </div>
                      <div className="mb-4">
                         <h4 className="text-xs font-bold opacity-50 mb-2 uppercase">Shape</h4>
                         <div className="grid grid-cols-4 gap-2">
                             {SHAPES.slice(0, 12).map((s,i) => <button key={i} onClick={() => onUpdate(note.id, {shape: s})} className={`h-8 bg-current opacity-20 ${s}`} />)}
                         </div>
                      </div>
                      <div className="mb-4">
          ...
          {/* FIXED TOOLBAR */}
          <div className={`fixed bottom-0 left-0 right-0 z-[110] p-2 flex gap-1 justify-center border-t border-black/5 pb-[env(safe-area-inset-bottom)] ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`}>
             <div className="flex gap-1 overflow-x-auto no-scrollbar max-w-full px-2">
                ...
                <button onMouseDown={(e) => {e.preventDefault(); format('formatBlock', 'H2')}} className="px-3 font-bold hover:bg-black/5 rounded-lg flex-shrink-0">H1</button>
                <button onMouseDown={(e) => {e.preventDefault(); format('formatBlock', 'H3')}} className="px-3 font-bold text-sm hover:bg-black/5 rounded-lg flex-shrink-0">H2</button>
                <div className="w-px h-6 bg-current opacity-20 mx-2 self-center flex-shrink-0"/>
