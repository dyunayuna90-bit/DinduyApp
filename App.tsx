...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark }: any) => {
  ...
  // Viewport Logic for Mobile Keyboard with Top Offset Fix
    ...
    const handleResize = () => {
      if (window.visualViewport) {
        setViewportStyle({
          ...
          top: `${window.visualViewport.offsetTop}px` // Fixes the gap issue
        });
      } else {
  ...
  // Check formatting state on interaction
  const checkFormats = () => {
    const formats = [];
  ...
  return (
    <div 
        style={viewportStyle}
        className={`fixed left-0 w-full z-[100] flex flex-col ${isDark ? 'bg-[#121212]' : 'bg-[#fdfcf4]'}`}
    >
...
const Dashboard = ({ 
  ...
  // Check if any selected item is a folder to hide "Move" button
