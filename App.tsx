  ...
  return (
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) handleExitSelection(); }}>
      <header className="mb-8 flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
         <button onClick={toggleSidebar} className={`p-3 rounded-xl transition-colors ${isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-white hover:bg-zinc-100'}`}>
            <Menu size={24} />
         </button>
         <div className={`flex-1 relative rounded-2xl flex items-center px-4 py-3 transition-colors ${isDark ? 'bg-zinc-900 focus-within:bg-zinc-800' : 'bg-white focus-within:bg-zinc-50'}`}>
         ...
         <button onClick={() => isSelectionMode ? handleExitSelection() : setIsSelectionMode(true)} className={`p-3 rounded-xl transition-colors ${isSelectionMode ? 'bg-indigo-600 text-white' : isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-white hover:bg-zinc-100'}`}>
            <CheckSquare size={24} />
         </button>
      </header>
