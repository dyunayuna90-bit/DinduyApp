...
import { 
  ...
  Check, Minimize2, Maximize2, Search, Heart, Menu, Download, Upload, CheckSquare, AlertCircle, Plus
} from 'lucide-react';
            ...
            >
                ...
                <div className="mb-6">
                    ...
                    <input 
                        ...
                        className={`w-full p-3 rounded-xl text-lg font-medium border-2 outline-none ${isDark ? 'bg-zinc-800 border-zinc-700 focus:border-indigo-500' : 'bg-zinc-50 border-zinc-200 focus:border-indigo-500'}`}
                    />
                </div>
                        ...
                        {Object.keys(colors).map(c => {
                             ...
                             return (
                                <button 
                                  ...
                                  className={`w-10 h-10 rounded-full border-2 ${bgClass} ${isActive ? 'border-indigo-600 scale-110 ring-2 ring-indigo-200' : 'border-transparent'}`}
                                />
                             )
                        })}
                    </div>
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark }: any) => {
  ...
  // FIX: Uncontrolled Component Logic to prevent Cursor Jumping
  useEffect(() => {
  ...
  }, []); // Only run on mount. Subsequent updates handled by onInput without re-rendering innerHTML.

  return (
    // FIX: Flexbox Layout 100dvh for Keyboard handling
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] ${isDark ? 'bg-[#121212]' : 'bg-[#fdfcf4]'}`}>
       
       {/* 1. FLEX-NONE HEADER (Static) */}
                ...
                <div className={`absolute top-14 right-0 w-80 max-h-[400px] overflow-y-auto p-4 rounded-3xl shadow-xl z-50 border-2 ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`}>
                   <div className="mb-4">
                      ...
                      <div className="flex flex-wrap gap-2">{Object.keys(colors).map(c => {
                          ...
                          return <button key={c} onClick={() => onUpdate(note.id, {color: c})} className={`w-8 h-8 rounded-full border border-black/10 ${bgClass} ${isActive ? 'ring-2 ring-indigo-500 scale-110' : ''}`} />
                      })}</div>
                   </div>
                   <div className="mb-4">
       ...
       {/* 2. FLEX-1 SCROLLABLE CONTENT */}
       <div className={`flex-1 overflow-y-auto px-6 py-6 ${themeClass}`}>
          <div className="max-w-4xl mx-auto pb-24">
              ...
              <div 
                ...
                className="outline-none text-xl leading-relaxed min-h-[50vh] empty:before:content-['Type_something...'] empty:before:opacity-50 text-left"
                dir="ltr"
                // REMOVED dangerouslySetInnerHTML to fix cursor jumping
              />
          </div>
       </div>

       {/* 3. FLEX-NONE STATIC TOOLBAR (Pushes up with keyboard) */}
       <div className={`flex-none w-full p-2 border-t border-black/5 pb-[env(safe-area-inset-bottom)] ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-200'}`}>
          <div className="flex gap-1 overflow-x-auto no-scrollbar justify-start md:justify-center px-2">
  ...
  return (
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) handleExitSelection(); }}>
      <header className="mb-8 flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
         ...
         <button onClick={() => isSelectionMode ? handleExitSelection() : setIsSelectionMode(true)} className={`p-3 rounded-xl transition-colors ${isSelectionMode ? 'bg-indigo-600 text-white' : isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-white hover:bg-zinc-100 border border-zinc-200'}`}>
            <CheckSquare size={24} />
         </button>
      </header>
