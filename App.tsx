...
// --- Folder Item (REWRITTEN: Single Container) ---
...
const FolderItem = ({ 
  ...
  return (
    <motion.div 
       layout 
       // CRITICAL: The entire expanded content must be inside this div
    ...
    >
        <div className="relative w-full h-full rounded-3xl">
            {/* Background Layer for Swipe Delete */}
            ...
            <motion.div
                ...
                // CRITICAL: Background color is applied here, on the main container
            ...
            >
                ...
                {/* 2. EXPANDED GRID SECTION - INSIDE THE SAME CONTAINER */}
                <AnimatePresence>
                    {isExpanded && (
                        <motion.div 
                            initial={{ opacity: 0 }} 
                            animate={{ opacity: 1 }} 
                            exit={{ opacity: 0 }}
                            className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4"
                        >
                            {notes.map((note: Note) => (
                                <NoteCard 
                            ...
                            {/* 3. ADD BUTTON - INSIDE THE SAME CONTAINER */}
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, onLongPress }: any) => {
  ...
  // Ensure solid color usage even in folder (use the note's own color theme)
  const themeClass = colors[note.color] || colors.slate;
  const [isPeeking, setIsPeeking] = useState(false);
  const lp = useLongPress(() => { if(onLongPress) onLongPress(); });

  // State reset for Peeking
  useEffect(() => {
      if (!isPeeking) {
          // Reset any internal interactions if needed in future
      }
  }, [isPeeking]);
